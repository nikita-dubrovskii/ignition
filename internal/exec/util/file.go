...
func (u Util) PerformFetch(f FetchOp, abort <-chan int) error {
	const (
		CreateTempFile = iota
		ChangePermission
		FetchFile
	...
	for stage := 0; stage != done; stage++ {
		select {
		case <-abort:
			return fmt.Errorf("%q: %v", path, ignerrors.ErrFetchCancelled)
		default:
			switch stage {
			case CreateTempFile:
...
func (u Util) processFetchOp(msg string, path string, op FetchOp, abort <-chan int) error {
	select {
	case <-abort:
		return ignerrors.ErrFetchCancelled
	default:
		fetcher := func() error {
